...
/// The 5x7 glyph table, shared with the kernel's framebuffer console. Two
/// copies diverge silently: a character added on one side renders as the
...
fn draw_text_at(
    ...
    for (index, character) in text.iter().copied().enumerate() {
        let rows = font::glyph(font::normalise(character));
